...
def main():
    ...
    # wandb.init(project='clean_properties_run', config=args, name=args.name, id =args.id)
    ...
    resume_step = 0
    path = "/projectnb/aclab/tranhp/test_properties/transformers/examples/pytorch/language-modeling/bert5e-5/0_"
    ####
    for epoch in range(args.starting_epoch,args.num_train_epochs):
        current_path = 10000*(epoch+1)
        checkpoint_path = path + str(current_path)
        ...
        for step, batch in enumerate(iterator):
            iteration +=1
            # if iteration % 100 ==0:
            #     print("currently in iteration:", iteration)
            ...
            if completed_steps >= args.max_train_steps:
                break
        grad = [torch.zeros_like(p) for p in model.parameters()]
        i = 0
        with torch.no_grad():
            for p in model.parameters():
                grad[i].copy_(p.grad)
                grad[i] = torch.div(grad[i], iteration)
                i+=1
        dic = {'fullBatch_loss': total_loss/iteration, 'fullbatch_gradient': grad }
        store_name ="/projectnb/aclab/tranhp/test_properties/transformers/examples/pytorch/language-modeling/bigBatch_checkpoint/0_" + str(current_path)+".pth.tar"
