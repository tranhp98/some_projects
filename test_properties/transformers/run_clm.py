...
def main():
    ...
    # eval_dataloader = LoadIt(None, root_dir = '/projectnb/aclab/tranhp/evalDataloader_pile/', max_workers = 1)
    # Optimizer
    ...
    if args.check_opt:
        print('in check_opt')
        path_opt = "/projectnb/aclab/tranhp/test_properties/transformers/examples/pytorch/language-modeling/pile8/0_2500000.pth.tar"
        opt = torch.load(path_opt)
        prev_grad = opt['current_grad']
        prev_loss = opt['current_loss']
        # path_prev = "/projectnb/aclab/tranhp/test_properties/transformers/examples/pytorch/language-modeling/bert5e-5/0_560000.pth.tar"
    ...
    for epoch in range(args.starting_epoch, args.num_train_epochs):
        ...
        for i in range(resume_step+1, args.max_step):
            ...
            # print("step", step)
            iteration+=1
            prev_batch = {key: value.to(device) if isinstance(value, torch.Tensor) else value for key, value in train_dataloader[i-1].items()}
            batch = {key: value.to(device) if isinstance(value, torch.Tensor) else value for key, value in train_dataloader[i].items()}
            with accelerator.accumulate(model):
                ...
                if iteration >1 and lr_scheduler.get_lr()[0] > 5e-8:
                    ...
                    # print("current_convexity_gap", current_convexity_gap)
                    ...
                    # print("convexity_gap", convexity_gap)
                ...
                if iteration % 10000 == 0 or (iteration ==1 and resume_step == 0):
                    # # prev_loss = f(w_{t-1},x_{t-1}), current_loss = f(w_{t},x_{t-1})
                    dic = { 'current_loss':current_loss, 'current_grad': current_grad, 'prev_loss': prev_loss,
                           ...
                           'total_loss': total_loss, 'prev_grad': prev_grad, 'prev_param': prev_param}
